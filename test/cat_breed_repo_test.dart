...
void main() {
  ...
  group('CatBreedRepo', () {
    ...
    group('getBreeds', () {
      test('should return success with list of cat breeds when API call succeeds',
          () async {
        // Arrange - Set up test data and mock behavior
        final mockBreeds = createMockCatBreeds();

        // Tell the mock what to return when getBreeds is called
        when(() => mockApiService.getBreeds(10, 0))
      ...
      test('should handle different limit and page parameters', () async {
        ...
        when(() => mockApiService.getBreeds(20, 2))
            .thenAnswer((_) async => mockBreeds);
    ...
    group('searchBreeds', () {
      test('should return success with filtered breeds when search succeeds',
          () async {
        // Arrange
        ...
        when(() => mockApiService.searchBreeds(query, attachImage))
            .thenAnswer((_) async => mockBreeds);
        ...
        verify(() => mockApiService.searchBreeds(query, attachImage)).called(1);
      });

      test('should return failure when search API call throws exception',
          () async {
        // Arrange
        const query = 'test';
        const attachImage = 1;
      ...
      test('should handle search with attachImage parameter set to 0',
          () async {
        // Arrange
        final mockBreeds = createMockCatBreeds();
        const query = 'aege';
      ...
      test('should return empty list when no breeds match search query',
          () async {
        // Arrange
      ...
      test('should handle case-sensitive search queries', () async {
        ...
        when(() => mockApiService.searchBreeds(queryUpperCase, attachImage))
            .thenAnswer((_) async => mockBreeds);

        // Act
        final result =
            await catBreedRepo.searchBreeds(queryUpperCase, attachImage);

        // Assert
        expect(result, isA<Success<List<CatBreed>>>());
        verify(() => mockApiService.searchBreeds(queryUpperCase, attachImage))
            .called(1);
      });
    });
  });
